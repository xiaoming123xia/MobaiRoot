...
::: details 将会了解 MobaiRoot-Ultra 安装过程
[[toc]]
:::
...
1. 适用于 GKI 2.0 设备，如小米、红米等（不包括内核修改的制造商，如三星、魅族、一加、真我和 OPPO）
...
::: tip
适用于 GKI 2.0 设备，如小米、红米等（不包括内核修改的制造商，如三星、魅族、一加、真我和 OPPO）
:::
...
面向希望将 MobaiRoot 集成到自己内核构建中的高级用户
...
```sh [bash]
curl -LSs "https://raw.githubusercontent.com/MobaiRoot-Ultra/MobaiRoot-Ultra/main/kernel/setup.sh" | bash -s susfs-dev
```
...
1. 系统更新后重启前
   - OTA 安装后不要立即重启，将 MobaiRoot 安装到第二插槽
   - 打开 MobaiRoot 管理器，在**刷入/修补内核**界面选择 **GKI/non_GKI install**，选择您的 `AnyKernel3` 内核 `zip` 文件，选择与当前运行插槽相对的插槽然后刷入重启。
