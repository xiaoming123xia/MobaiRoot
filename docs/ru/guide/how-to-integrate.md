...
MobaiRoot можно интегрировать как в ядра _GKI_, так и в _non-GKI_. Выполнен бэкпорт до версии _4.14_.

<!-- It should be 3.4, but backslashxx's syscall manual hook cannot use in MobaiRoot-->
...
3. **Tracepoint Hook:**
   ...
   - Метод представлен в MobaiRoot начиная с коммита [49b01aad](https://github.com/MobaiRoot-Ultra/MobaiRoot-Ultra/commit/49b01aad74bcca6dba5a8a2e053bb54b648eb124).
...
Команда для добавления MobaiRoot (писать в корневой папке исходников ядра):

```sh
curl -LSs "https://raw.githubusercontent.com/MobaiRoot-Ultra/MobaiRoot-Ultra/main/kernel/setup.sh" | bash -s main
```
...
Команды для добавления MobaiRoot в дерево исходников вашего ядра:
...
```sh
curl -LSs "https://raw.githubusercontent.com/MobaiRoot-Ultra/MobaiRoot-Ultra/main/kernel/setup.sh" | bash -s builtin
```

### GKI / Built-in ядро с поддержкой susfs (экспериментально)

```sh
curl -LSs "https://raw.githubusercontent.com/MobaiRoot-Ultra/MobaiRoot-Ultra/main/kernel/setup.sh" | bash -s susfs-{{branch}}
```
...
- `main` (susfs-main)
- `test` (susfs-test)
- конкретная версия (например: susfs-1.5.7, проверить доступные варианты можно в разделе [branches](https://github.com/MobaiRoot-Ultra/MobaiRoot-Ultra/branches))
